...
'''
    ...
    3. find out which cc present in aura region
...
def tableFromAura(indx,originalImage,auraImageName):
    ...
    try:
        ...
        cv2.imwrite(basePath+expNo+ "/delMe/lineResults//"+ str(indx)+'_aura.jpg', auraImage)
        #print(tempImg.shape)
        ...
        #cv2.imwrite(basePath+expNo+ "/delMe/lineResults//" + str(indx)+"_"+'duplicateImage_label.jpg',255 * duplicateImage)

        #duplicateImage=255*duplicateImage
        uniueID=np.unique(duplicateImage)
        print("\n\t uniueID=",len(uniueID))
        print("\n\t uniueID label=",len(labels))
        #input("uniueID=")
        nonPresentID=[]#
        allCompId=np.unique(labels)
        #print "\n\t duplicateImage =", np.unique(duplicateImage)
        print "\n\t tempAura1 unique =", np.unique(tempAura1),"\t ",type(tempAura1),"\t shape=",tempAura1.shape
        print "\n\t duplicateImage =",np.unique(duplicateImage)
        print "\n\t unique labels=",allCompId
        print "\n\t uniueID==",uniueID
        print "\n\t allCompId=",allCompId
        print "\n\t labels shape=",labels.shape,"\t ",type(labels)
        #show(labels)
        #input("check")
        '''
        ...
        for comID in np.unique(allCompId):
            if 1 :#comID in uniueID:
                if comID > 0:
                    ...
                    #cv2.imwrite("C:\Users\Lenovo\PycharmProjects\wsl\publicationData\8\delMe\lineResults2\\"+str(indx)+"_" +str(comID)+".jpg",crop)
                    cv2.imwrite(basePath+"\\2\delMe\lineResults2\\"+str(indx)+"_" +str(comID)+"_crop_.jpg",crop)

        cv2.imwrite(basePath + expNo + "/delMe/nonFocus//" + str(indx) + "RealFocus.jpg", 255*tempCopy)
        cv2.imwrite(basePath + expNo + "/delMe/nonFocus//" + str(indx) + "nonFocus.jpg", tempCopy1)
        cv2.imwrite(basePath + expNo + "/delMe/nonFocus//" + str(indx) + "nonFocus.jpg", 255*tempCopy1)
        cv2.imwrite(basePath+expNo+ "/delMe/nonFocus//"+str(indx)+'Focus.jpg', 255-255*extract)
        cv2.imwrite(basePath+expNo+ "/delMe/nonFocus//"+ str(indx) + '_original.jpg', originalImg)
        #cv2.imwrite(basePath+expNo+ "/delMe/nonFocus//"+ str(indx) + 'rFocus.jpg', originalImg-(255-(255*extract)))
        #tableLines(indx, 255*tempCopy)
        cv2.imwrite(basePath + expNo + "/delMe/nonFocus//" + str(indx) + 'rFocus.jpg',tempOriginal - focusImage)

        extractTable=255-255*extract

        extractTable1 =cv2.resize(extractTable,(500,500))
        cv2.imwrite(basePath + expNo + "/delMe/lineResults2//" + str(indx) + 'realFocus.jpg',tempOriginal - focusImage)

        #call(extractTable,255*tempCopy,originalImg, basePath, expNo,indx)
        #input("check")
...
for indx in range(6,13):

    print("\n\t indx=",indx)

    try:
        originalImage=str(indx)+"_original_1.png"
        auraImageName=str(indx)+"_cam_map_11.png"
